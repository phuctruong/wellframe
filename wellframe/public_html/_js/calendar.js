...
function stick(day, month, year) {
  year=year.toString();
  month++;
  window.thing.value=month.toString()+'/'+day.toString()+'/'+year;
...
function popCalendar(thing, graphics_url) {
  window.thing=thing;
  calendarWindow=window.open("", "calendarWindow", 'height=260,width=230,top=200,left=300')
...
function drawCalendar(day, graphics_url) {
  ...
  calendarWindow.document.write('<html><title>Calendar</title><LINK REL=STYLESHEET TYPE=text/css HREF=../../../css/ecs.css>');
  calendarWindow.document.write('<body bgcolor=#FFFFCC><form name=flip>');
  calendarWindow.document.write('<table border=0 width=100% bgcolor= cellpadding=0 cellspacing=0><tr class=trtop>');
  ...
  calendarWindow.document.write('<td align=center><font class=tabletop><select name=month onChange="window.opener.jump('+day.getFullYear()+')";><option value='+writeMonth(day.getMonth())+ '>'+writeMonth(day.getMonth())+'</option><option value="0">January<option value="1">February<option value="2">March<option value="3">April<option value="4">May<option value="5">June<option value="6">July<option value="7">August<option value="8">September<option value="9">October<option value="10">November<option value="11">December</select>');
...
calendarWindow.document.write('&nbsp;&nbsp;&nbsp;&nbsp;' +day.getFullYear()+'</font></td>');
  ...
  calendarWindow.document.write('<table border=0 align=center><tr><td><font class=fieldlabel>Sun</font></td><td><font class=fieldlabel>Mon</font></td><td><font class=fieldlabel>Tue</font></td><td><font class=fieldlabel>Wed</font></td><td><font class=fieldlabel>Thu</font></td><td><font class=fieldlabel>Fri</font></td><td><font class=fieldlabel>Sat</td></tr></font>');
  ...
  for (days=1; days<=daysInMonth[day.getMonth()+1]; days++) {
    ...
    if (days==today.getDate() && today.getMonth()==day.getMonth()) {
      calendarWindow.document.write('<td align=center class=trtop><a class=calendar href="javascript:function cal(){window.close();}" onclick=window.opener.stick('+days+','+day.getMonth()+','+day.getFullYear()+')>'+days+'</a></td>\n');
    } else {
      calendarWindow.document.write('<td align=center><a class=calendar href="javascript:function cal(){window.close();}" onclick="window.opener.stick('+days+','+day.getMonth()+','+day.getFullYear()+')">'+days+'</a></td>\n');
    }
    currday=(currday+1)%7;
  }
  ...
  calendarWindow.document.write('<p align=center><h4>Choose a date to be entered into field.</h4>');
